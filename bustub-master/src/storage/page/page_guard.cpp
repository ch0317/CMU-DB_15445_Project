...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
                             ...
                             std::shared_ptr<DiskScheduler> disk_scheduler, bool already_locked)
    : page_id_(page_id),
...
void ReadPageGuard::Flush() {
  ...
  if (frame_->is_dirty_) {
    ...
    reqs.push_back({true, frame_->GetDataMut(), page_id_, std::move(promise)});
...
void WritePageGuard::Drop() {
  ...
  // Release the exclusive write latch first, then update replacer under bpm_latch
